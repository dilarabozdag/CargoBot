...
public class LevelReader {
	...
	//get the width size of the table
	...
	private void getDataForInitial(String[] lineElement) {
		...
		for (int i = 1; i < lineElement.length; i++) {
			...
			int yIndex = Integer.parseInt(boxElement[1]);
			
			Box box = new Box(boxName);
			box.setPosition(leftLimit + xIndex * (box.size + box.space), downLimit - (7 - yIndex) * box.size);
